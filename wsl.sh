...
if [[ "$INSTALL_DIR" =~ " " ]]; then echo ????????Miniconda??????????????????¡¤??????????? && exit; fi
...
if [ "$conda_exists" == "F" ]; then
    echo "????? $MINICONDA_DOWNLOAD_URL ????Miniconda?? $INSTALL_DIR/miniconda_installer.sh"
