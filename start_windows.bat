...
@rem download conda
if "%conda_exists%" == "F" (
	echo ????? %MINICONDA_DOWNLOAD_URL% ????Miniconda?? %INSTALL_DIR%\miniconda_installer.exe
	...
	if not defined output (
		echo miniconda_installer.exe??§µ????????????
	...
	echo ?????Miniconda????? %CONDA_ROOT_PREFIX%
	start /wait "" "%INSTALL_DIR%\miniconda_installer.exe" /InstallationType=JustMe /NoShortcuts=1 /AddToPath=0 /RegisterPython=0 /NoRegistry=1 /S /D=%CONDA_ROOT_PREFIX%

	@rem test the conda binary
	echo Miniconda?·Ú??
	call "%CONDA_ROOT_PREFIX%\_conda.exe" --version || ( echo. && echo ?????Miniconda?? && goto end )
...
if not exist "%INSTALL_ENV_DIR%" (
	...
	call "%CONDA_ROOT_PREFIX%\_conda.exe" create --no-shortcuts -y -k --prefix "%INSTALL_ENV_DIR%" -c https://repo.anaconda.com/pkgs/main/ python=3.11 || ( echo. && echo ????Conda???????? && goto end )
)

@rem check if conda environment was actually created
if not exist "%INSTALL_ENV_DIR%\python.exe" ( echo. && echo Conda????¦Ä?????? && goto end )
