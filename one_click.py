...
def update_pytorch():
    ...
    if "+cu118" in torver:
        install_cmd = f"{base_cmd} --index-url https://mirror.sjtu.edu.cn/pytorch-wheels/cu118"
    elif "+cu" in torver:
        install_cmd = f"{base_cmd} --index-url https://mirror.sjtu.edu.cn/pytorch-wheels/cu121"
    elif "+rocm" in torver:
        install_cmd = f"{base_cmd} --index-url https://mirror.sjtu.edu.cn/pytorch-wheels/rocm6.1"
    elif "+cpu" in torver:
        install_cmd = f"{base_cmd} --index-url https://mirror.sjtu.edu.cn/pytorch-wheels/cpu"
...
def update_requirements(initial_installation=False, pull=True):
    ...
    if not os.path.exists(os.path.join(script_dir, ".git")):
        run_cmd(
            "git init -b main && git remote add origin https://gitee.com/touchnight/text-generation-webui && "
            "git fetch && git symbolic-ref refs/remotes/origin/HEAD refs/remotes/origin/Chinese && "
            "git reset --hard origin/Chinese && git branch --set-upstream-to=origin/Chinese && git submodule update --init --recursive",
            environment=True,
            assert_success=True
        )
