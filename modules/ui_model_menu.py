                ...
                with gr.Blocks():
                    with gr.Row():
                        with gr.Column():
                            ...
                            shared.gradio['n_ctx'] = gr.Number(label="上下文大小", precision=0, step=256, value=shared.args.n_ctx, info='上下文长度。⚠️如果你不能加载模型，试试调低这个值。')
                            shared.gradio['max_seq_len'] = gr.Number(label='max_seq_len', precision=0, step=256, value=shared.args.max_seq_len, info='上下文长度。⚠️如果你不能加载模型，试试调低这个值。')
