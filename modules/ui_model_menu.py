                ...
                with gr.Blocks():
                    with gr.Row():
                        with gr.Column():
                            ...
                            shared.gradio['n_gpu_layers'] = gr.Slider(label="GPU层数", minimum=0, maximum=256, value=shared.args.n_gpu_layers, info='必须要设为大于0的值，你的GPU才会被使用。⚠️如果你不能加载模型，试试调低这个值。')
                            ...
                            shared.gradio['n_ctx'] = gr.Number(label="上下文大小", precision=0, step=256, value=shared.args.n_ctx, info='上下文长度。⚠️如果你不能加载模型，试试调低这个值。常见值：2048，4096，8192，16384，32768。')
                            shared.gradio['max_seq_len'] = gr.Number(label='max_seq_len', precision=0, step=256, value=shared.args.max_seq_len, info='上下文长度。⚠️如果你不能加载模型，试试调低这个值。常见值：2048，4096，8192，16384，32768。')
                            shared.gradio['cache_type'] = gr.Dropdown(label="cache_type", choices=['fp16', 'q8_0', 'q4_0', 'fp8', 'q8', 'q6', 'q4'], value=shared.args.cache_type, info='可选项：llama.cpp - fp16, q8_0, q4_0; ExLlamaV2 - fp16, fp8, q8, q6, q4.')
                            ...
                            shared.gradio['compute_dtype'] = gr.Dropdown(label="计算数据类型", choices=["bfloat16", "float16", "float32"], value=shared.args.compute_dtype, info='用于以4位量化加载。')
                            shared.gradio['quant_type'] = gr.Dropdown(label="量化类型", choices=["nf4", "fp4"], value=shared.args.quant_type, info='用于以4位量化加载。')
                            shared.gradio['attention_sink_size'] = gr.Number(label="注意力下沉大小", value=shared.args.attention_sink_size, precision=0, info='StreamingLLM：下沉词符的数量。仅在修剪后的提示词不与旧提示词前缀相同时使用。')
                            shared.gradio['num_experts_per_token'] = gr.Number(label="每个词符的专家数量", value=shared.args.num_experts_per_token, info='仅适用于像Mixtral这样的混合专家模型。')

                        with gr.Column():
                            shared.gradio['tensorcores'] = gr.Checkbox(label="tensorcores", value=shared.args.tensorcores, info='仅限N卡：使用不带GGML_CUDA_FORCE_MMQ的llama-cpp-python。这在新款显卡上可能可以提高性能。')
                            ...
                            shared.gradio['torch_compile'] = gr.Checkbox(label="用torch编译模型", value=shared.args.torch_compile, info='使用torch.compile编译模型以提高性能。')
                            ...
                            shared.gradio['cfg_cache'] = gr.Checkbox(label="CFG缓存", value=shared.args.cfg_cache, info='配合CFG使用此加载器时，必须勾选此项。')
