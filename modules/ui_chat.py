...
def create_ui():
    ...
    with gr.Tab('聊天', id='Chat', elem_id='chat-tab'):
        with gr.Row(elem_id='past-chats-row', elem_classes=['pretty_scrollbar']):
            with gr.Column():
                with gr.Row(elem_id='past-chats-buttons'):
                    shared.gradio['branch_chat'] = gr.Button('分支', elem_classes='refresh-button', interactive=not mu)
                ...
                shared.gradio['search_chat'] = gr.Textbox(placeholder='搜索聊天...', max_lines=1, elem_id='search_chat')
                
                with gr.Row(elem_id='delete-chat-row', visible=False) as shared.gradio['delete-chat-row']:
                    shared.gradio['delete_chat-cancel'] = gr.Button('取消', elem_classes=['refresh-button', 'focus-on-chat-input'])
                    shared.gradio['delete_chat-confirm'] = gr.Button('确认', variant='stop', elem_classes=['refresh-button', 'focus-on-chat-input'])

                with gr.Row(elem_id='rename-row', visible=False) as shared.gradio['rename-row']:
                    shared.gradio['rename_to'] = gr.Textbox(label='重命名为：', placeholder='新名称', elem_classes=['no-background'])
                    with gr.Row():
                        shared.gradio['rename_to-cancel'] = gr.Button('取消', elem_classes=['refresh-button', 'focus-on-chat-input'])
                        shared.gradio['rename_to-confirm'] = gr.Button('确认', elem_classes=['refresh-button', 'focus-on-chat-input'], variant='primary')
    ...
    with gr.Tab('指令模板'):
        ...
        with gr.Row():
            with gr.Column():
                ...
                shared.gradio['instruction_template_str'] = gr.Textbox(value='', label='指令模板', lines=24, info='这个选项会自动检测，您通常不需要手动更改。在指令和聊天指令模式下使用。', elem_classes=['add_scrollbar', 'monospace'])
                with gr.Row():
